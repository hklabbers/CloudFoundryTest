...
SCENARIO("Set of parameters; Minute is missing", "[ConversionParameters]") {
    GIVEN("A JSON with the needed parameters (except year)")
        GIVEN("Parameters are in the correct format") {
            std::string jsonString = "{ \"TimeZoneFrom\":\"Europe/London\", \"TimeZoneTo\":\"Europe/Amsterdam\","
                    "\"Year\":2017,\"Month\":3,\"Day\":20,\"HourIn24H\":15}";
            ...
            WHEN("JSON is parsed") {
                ...
                REQUIRE(parameters.getParameterErrors().at(0) == "Minute - parameter not given.");
            }
        }
}

SCENARIO("Set of parameters; Minute is not an integer", "[ConversionParameters]") {
    GIVEN("A JSON with all the needed parameters") {
        ...
        WHEN("JSON is parsed") {
            ...
            REQUIRE(parameters.getTimeZoneTo() == "Europe/Amsterdam");
            REQUIRE(parameters.getParameterErrors().size() == 1);
            REQUIRE(parameters.getParameterErrors().at(0) == "Minute - parameter is not an integer.");
        }
    }
}

SCENARIO("Set of parameters; Minute is 60", "[ConversionParameters]") {
    GIVEN("A JSON with all the needed parameters") {
        std::string jsonString = "{ \"TimeZoneFrom\":\"Europe/London\", \"TimeZoneTo\":\"Europe/Amsterdam\","
                "\"Year\":2017,\"Month\":3,\"Day\":20,\"HourIn24H\":15,\"Minute\":60}";
        std::stringstream is{jsonString, std::ios::in};
        ...
        WHEN("JSON is parsed") {
            ...
            REQUIRE(parameters.getTimeZoneTo() == "Europe/Amsterdam");
            REQUIRE(parameters.getParameterErrors().size() == 1);
            REQUIRE(parameters.getParameterErrors().at(0) == "Minute - parameter should be in the range 0 to 59.");
        }
    }
}
